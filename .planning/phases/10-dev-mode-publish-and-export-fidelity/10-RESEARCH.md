...
Implement Phase 10 as three connected layers:

1. `editor-core` Dev Mode state and handoff contracts: inspect reports, code snippets, ready-for-dev markers, version diffs, and asset download metadata must live inside `ProjectBundle` and be validated on reload.
2. A local export materialization worker: real files should be created from stored state through a Node worker path, not from live iframe DOM. HTML, ZIP, PNG, PDF, PPTX, GIF, and MP4 should produce deterministic artifacts plus visual/fidelity diagnostics.
3. Web studio workflow: expose Dev Mode, ready-for-dev, version diff, asset download, publish preview, export queue, and code roundtrip in the product shell with browser tests.
