...
| 2 | Validate agent output and close corruption paths | this changeset | Added agent-output ingestion helper, strict selected-region allowlists, rejected-run diagnostics, and persisted integrity checks. |
...
- `packages/editor-core/src/agent-output.ts` - Local-first context package creation, agent output parsing/ingestion, scope validation, and rejected-run persistence.
...
- `packages/editor-core/src/integrity.ts` - Validates persisted agent context packages, outputs, and runs on load/serialize.
- `packages/editor-core/src/index.ts` - Exports the agent output module.
- `packages/editor-core/src/__tests__/persistence.test.ts` - Adds corrupt persisted agent-output reference rejection cases.
...
- `pnpm --filter @kdesign/editor-core test -- src/__tests__/agent-output.test.ts src/__tests__/persistence.test.ts src/__tests__/variations.test.ts` passed: 15 files, 80 tests.
- `pnpm exec playwright test apps/web/tests/phase-08-agent-output.spec.ts` passed: 2 browser tests.
- `npx tsc --noEmit`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (16 files, 88 tests), and `pnpm e2e` (24 browser tests) passed.
