...
| ID | Decision | Reason | Impact |
|---|---|---|---|
| D-09-18 | DATA-01 completion in Phase 09 means an account-shaped, server-portable sync envelope and deterministic local adapter, not production auth, billing, permissions, or object-storage hardening. | The roadmap requires local-to-hosted portability, but full hosted product ops belong later. | Implement sync metadata, remote document ids, revision cursors, conflict diagnostics, and import/export tests against a local/mock adapter. |
