...
The quality benchmark is Claude Design-level product completeness, but the runtime must be agent-agnostic. The user must be able to continue the same design work from Codex, Claude Code, Cursor, local agents, or web agents without changing the artifact model.
