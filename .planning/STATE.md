...
- Phase 11 plan hardening after independent review tightened 11-02 so COLL-01 cannot be overclaimed as metadata-only collaboration. Wave 2 must now prove two collaborators/two sessions opening the same shared document id, shared/reload-visible state, role permission enforcement for viewer/commenter/editor/owner, and permission/audit records for denied actions. Full realtime sockets/CRDT remain deferred, but same-document shared access semantics are in scope.
